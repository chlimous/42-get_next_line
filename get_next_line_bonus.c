...
{
	static t_static		persis[FD_MAX + 1];
	char				*buffer;
	ssize_t				bytesread;
	...
	while (bytesread > 0 && !is_eol(&persis[fd]))
	{
		...
		if (bytesread > 0)
			buffercat(&persis[fd], buffer, bytesread);
		if (!persis[fd].size || bytesread == -1)
			return (clearstatic(&persis[fd]), free(buffer), NULL);
	}
	return (free(buffer), build_line(&persis[fd]));
}
